...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function renderProfile() {
        ...
        resumeBtn.setAttribute('download', 'Vishuddhi_Jain_Resume.pdf');
    ...
    //         const hasLive = project.liveDemoUrl && project.liveDemoUrl.trim() !== '';
    ...
    //                     <h3 class="project-title">${hasLive
    //                       ? `<a class="project-title-link" href="${project.liveDemoUrl}" target="_blank" rel="noopener noreferrer" aria-label="${project.name} Live Demo">${project.name}</a>`
    //                       : `<span class="project-title-text" aria-hidden="false">${project.name}</span>`
    //                     }</h3>
    ...
    //                 console.log('Live demo clicked:', project.name);
    //             });
    //          }
    //         }
    //         });
...
function renderOtherProjects() {
    ...
    projects.other.forEach(project => {
        ...
        // use per-project themeColor if present, otherwise use the accent you provided
        const accent = (project.themeColor && project.themeColor.trim() !== '') ? project.themeColor : '#00A3FF';

        // set CSS variable on the card so all CSS that uses var(--accent-color) picks it up
        card.style.setProperty('--accent-color', accent);

        const hasLive = project.liveDemoUrl && project.liveDemoUrl.trim() !== '';

        // safely handle missing sourceCodeUrl
        const sourceLinkHtml = project.sourceCodeUrl
            ? `<a href="${project.sourceCodeUrl}" target="_blank" rel="noopener noreferrer" aria-label="${project.name} Source Code">
                 <i class="devicon-github-original"></i>
               </a>`
            : '';
        ...
        // image -> modal (keep your existing modal trigger)
        const projectImage = card.querySelector('.project-image');
        if (projectImage) {
        ...
        // analytics placeholder for live-demo clicks (optional)
        if (hasLive) {
            const titleLink = card.querySelector('.project-title-link');
            if (titleLink) {
                titleLink.addEventListener('click', () => {
